// Downloads Poppins (text) and Instrument Serif (display) and writes src/assets/fonts.css plus the woff2 files, so
...
// Poppins carries the reading text. Instrument Serif is the display face — the
// title cards, section headings and pull quotes — and ships in one weight,
// upright and italic, which is all a display face on this site needs.
const FAMILIES = [
  { family: 'Poppins', slug: 'poppins', query: 'Poppins:wght@300;400;500;600;700;800' },
  { family: 'Instrument Serif', slug: 'instrument-serif', query: 'Instrument+Serif:ital@0;1' },
];
...
if (!response.ok) {
  throw new Error(`Fonts: Google returned ${response.status} for the ${family} stylesheet`);
}
...
if (blocks.length === 0) {
  throw new Error(`Fonts: could not parse any ${family} @font-face blocks`);
}
 ...
 *
 * Self-hosted Poppins and Instrument Serif. Replaces the @import of fonts.googleapis.com that
